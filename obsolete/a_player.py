...
class Atari:
    ...
    def step(self, sess, action):
        ...
        # imshow(np.squeeze(processed_new_frame))
...
def train():
    ...
    with tf.Session() as sess:
        sess.run(init)
        #         trained_path = "output_Copy/"
        #         save_file = "my_model-7618748.meta"
        ...
        while frame_number < MAX_FRAMES:
            ...
            while epoch_frame < EVAL_FREQUENCY:
                ...
                for _ in range(MAX_EPISODE_LENGTH):
                    ...
                    # cv2.imshow('im', processed_new_frame)
                    frame_number += 1
                    epoch_frame += 1
                ...
                if len(rewards) % 10 == 0:
                    print(datetime.datetime.now() - time, ': ',
                          str(len(rewards)), ', ', str(best_reward), ', ',
                          np.mean(rewards[-100:]), ', ',
                          str(frame_number), ', ',
                          str(action_getter.eps), ', ',
                          np.mean(loss_list[-100:]))
